//! The router's collections are ordered, and that is checked rather than
//! intended.
//!
...
//! map two ways. The moment such a walk reaches a decision — which obstacle to
//! blame, which state to expand, which candidate wins a tie — the route stops
//! being a function of the drawing. `BTreeMap` and a sorted vector cost nothing
//! at the sizes a routing window holds, and they cannot do that.
//!
...
/// The collections the router may not name.
const FORBIDDEN: [&str; 2] = ["HashMap", "HashSet"];

/// The collection that answers for them, which the sweep expects to find.
const CANONICAL: &str = "BTreeMap";
...
#[test]
fn no_unordered_collection_appears_under_the_router() {
    ...
    assert!(
        offenders.is_empty(),
        "the router's collections must be ordered: {offenders:?}"
    );
}
...
fn the_sweep_can_see_what_it_is_looking_for() {
    ...
    // given, would pass the check above while a hash map sat in the search.
    let sources = router_sources();
    ...
    assert!(
        names_type(&text, CANONICAL),
        "the router keys its maps in a {CANONICAL}, so the sweep is reading the router"
    );
    ...
    assert!(names_type("let seen: HashSet<Cell> = ...", "HashSet"));
    assert!(names_type("(HashMap::new())", "HashMap"));
    assert!(!names_type("struct HashMapOfCells;", "HashMap"));
