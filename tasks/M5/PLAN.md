...
directory and generates the module list and registry into `OUT_DIR`. **No new
dependency**, so no Constitution §9 question, and `Cargo.toml` is untouched.

Both arms hold and each was reproduced independently by the reviewer in its own
`git archive` scratch copy: the file count, **and the rule actually running**.
The two arms are confirmed to have **no shared code path** — the test's own
`read_dir` against the build script's separate `rule_files()` — so the check is
not comparing a thing with itself.

**What this confirms, precisely, and what it does not:**

- **Confirmed: there is no shared registration list for lanes to queue on.** The
  merge hotspot this plan feared — *"a list that every lane in Phases 2 and 3
  must queue on"* — **does not exist.** `lint/mod.rs` is not a hotspot; the
  lane table's parenthetical *"(if T1 fails to remove it as one)"* is resolved
  and the entry is dropped from the hotspot list below.
- **Confirmed: the lane table stands as written.** Phase 2's A and B, and Phase
  3's C and D, own disjoint rule files and collide nowhere.
- **NOT changed: Phase 3 still opens at TWO lanes.** That is James's amendment
  and it was never conditional on this verdict — it is about coordination cost,
  not about the seam. A PASS makes widening *possible*; the frictionless-first-
  merges evidence is what makes it *earned*.
- **`inventory`/`linkme` were rejected on a measurement**: a distributed slice
  **does not remove the per-rule `mod` line**, so it would have bought a
  dependency for nothing.

**The FAIL branch was costed anyway, and the plan should keep the number**: it
would have been **one `mod` line per rule file, alphabetical, mechanical, no
match arm** — the *cheap* end of the range this section feared, not the
expensive end. Worth knowing that the downside was smaller than the plan
assumed, because it means a future seam decision of this shape can be made
faster.

**The cost the PASS carries, and it is a real one:** `cargo fmt --check`
**cannot see a rule file at all** — `rustfmt` walks the module tree from the
crate root and does not follow an `include!`. Measured twice and reproduced by
the reviewer: over a mangled rule file, `cargo fmt --check` exits 0 with zero
bytes of output while `rustfmt --check` on the same file exits 1. **One of the
...
**Merge hotspots, orchestrator-only:** `Cargo.toml`, `lib.rs`, `AGENT.md`,
`tests/command_surface.rs`, fixture `MANIFEST`, `spec/SPEC.md`, `kicli.toml`'s
`[rules]` table.

**`lint/mod.rs` is NOT on this list, and that is T1's result rather than an
omission.** The seam removed it as a hotspot: the module list is generated by
`build.rs` from the directory, so there is nothing for two lanes to collide in.
`crates/kicli/build.rs` itself **is** orchestrator-only from here — it is the
one file a rule author must never need to touch, and a lane editing it has
stopped being a rule author.
